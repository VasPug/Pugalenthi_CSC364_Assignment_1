...
def start_server():
    ...
    forwarding_table = util.read_csv("input/router_3_table.csv")
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        packet = util.receive_packet(connection, max_buffer_size,"./output/received_by_router_3.txt","3")
        ...
        if sending_port == "127.0.0.1":
            print("OUT:", payload)
            util.write_to_file("./output/out_router_3.txt", payload)
        else:
            util.write_to_file("./output/discarded_by_router_3.txt", new_packet)
