...
def start_server():
    ...
    forwarding_table = util.read_csv("input/router_4_table.csv")
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    router5_socket = util.create_socket('127.0.0.1', 8005)
    router6_socket = util.create_socket('127.0.0.1', 8006)
    ...
    while True:
        ...
        packet = util.receive_packet(connection, max_buffer_size,"./output/received_by_router_4.txt","4")
        ...
        if sending_port == "8005" and new_ttl > 0:
            ...
            util.write_to_file("./output/sent_by_router_4.txt", new_packet,"5")
        elif sending_port == "8006" and new_ttl > 0:
            print("sending packet", new_packet, "to Router 6")
            router6_socket.send(new_packet.encode())
            util.write_to_file("./output/sent_by_router_4.txt", new_packet,"6")
        elif sending_port == "127.0.0.1":
            print("OUT:", payload)
            util.write_to_file("./output/out_router_4.txt", payload)
        else:
            util.write_to_file("./output/discarded_by_router_4.txt", new_packet)
